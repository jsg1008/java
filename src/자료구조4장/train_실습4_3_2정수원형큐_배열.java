...
//--- 큐에 데이터를 인큐 ---//
	public  void enque(int x) throws OverflowIntQueue3Exception {
		...
		rear++;
		num++;
		// 책은 이렇게 코딩 (rear==capacity) rear = 0;
		rear = (rear+1)%capacity; //이렇게 코딩하면 reaar은 원형으로 계속 돌게 된다
...
//--- 큐에서 데이터를 디큐 ---//
	public int deque() throws EmptyIntQueue3Exception {
		// if (num<=0) 
		if (front == rear & isEmptyTag) throw new EmptyIntQueue3Exception();
		int result = que[front];
		front++;
		num--;
		// 책에서 쓴 표현. if (front == capacity) front=0;
